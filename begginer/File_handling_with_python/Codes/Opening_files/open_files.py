...
'''
file_objet=open(str(input('Enter the absolute file path : ')))
print('file successfully opened')
file_object.close()
'''
...
def pathprovider(n=int(input('press 1. for relative path \npress 2. for absolute paht\nEnter the option : '))):
	if n in (1,2):
		if n==1:
			return path=input('Enter the path : ')
		else :
			return path=input('Enter the path : ').replace('\\',r'\\')
	else: print('invalid input')

file_objet=open(pathprovider())
print('file exists')
file_object.close()
'''